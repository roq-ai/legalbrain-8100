...
import { getLegalResearches } from 'apiSdk/legal-researches';
import { LegalResearchInterface } from 'interfaces/legal-research';
import { getLegalInsights } from 'apiSdk/legal-insights';
import { LegalInsightInterface } from 'interfaces/legal-insight';
import { getLegalDocuments } from 'apiSdk/legal-documents';
import { LegalDocumentInterface } from 'interfaces/legal-document';
import { LegalInsightListPage as ShowCaseList } from 'pages/legal-insights';

function DashboardPage() {
  const { session } = useSession();

  const legalResearchStat = useSWR<PaginatedInterface<LegalResearchInterface>>(
    () => `/legal-researches`,
    () => getLegalResearches({ limit: 1, offset: 0 }),
  );

  const legalInsightStat = useSWR<PaginatedInterface<LegalInsightInterface>>(
    () => `/legal-insights`,
    () => getLegalInsights({ limit: 1, offset: 0 }),
  );

  const legalDocumentStat = useSWR<PaginatedInterface<LegalDocumentInterface>>(
    () => `/legal-documents`,
    () => getLegalDocuments({ limit: 1, offset: 0 }),
  );

  const statResults = [
    {
      title: 'Legal Researches',
      result: legalResearchStat?.data?.totalCount,
      isLoading: legalResearchStat.isLoading,
    },
    {
      title: 'Legal Insights',
      result: legalInsightStat?.data?.totalCount,
      isLoading: legalInsightStat.isLoading,
    },
    {
      title: 'Legal Documents',
      result: legalDocumentStat?.data?.totalCount,
      isLoading: legalDocumentStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`legal-insights`} passHref legacyBehavior>
                  <Button
